...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    NSString *Psd  = [UserDefUtils getStringForKey:@"Psd"];//第一次设置密码,需要验证的密码
    NSString *SavePsd  = [UserDefUtils getStringForKey:@"SavePsd"];//第二次设置密码
    
    
    if (Psd==nil) {//判断 有没有设置密码(这里存在输入第一次密码)
        
        //保存用户第一次绘制的密码
        [UserDefUtils saveString:password forKey:@"Psd"];
        ...
        if ([password  isEqualToString:Psd] && SavePsd==nil) {//判断第二次输入的密码,和第一次是否相同,做第二次确定判断
            ...
            [UserDefUtils saveString:password forKey:@"SavePsd"];
