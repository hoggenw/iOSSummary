...
-(void)initialDataSource {
    ...
    model.title = [NSString stringWithFormat:@"简体中文"];
    model.leadImageName = @"tabbar-icon-selected-1";
    model.cellAccessoryType = UITableViewCellAccessoryNone;
    [self.dataArray addObject: model];
    
    DefualtCellModel *model2 = [DefualtCellModel new];
    model2.title = [NSString stringWithFormat:@"English"];
...
- (void)initialUI {
    ...
    self.title = Localized(@"SetLanguage");
...
-(void)didselectedCell:(NSInteger)index {
    ...
    if (index == 0) {
        ...
        if ([laguageType hasPrefix:@"zh-Hans"]) {//开头匹配
        ...
        NSString *laguageType =[NSString stringWithFormat:@"%@", [[NSUserDefaults standardUserDefaults] objectForKey:@"appLanguage"]];
        if ([laguageType hasPrefix:@"en"]) {//开头匹配
