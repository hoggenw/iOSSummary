...
@interface YLPlayerView()<UIGestureRecognizerDelegate,UIApplicationDelegate>
...
#pragma mark - 初始化********************************************************
-(instancetype)initWithFrame:(CGRect)frame delegate:(id)delegate url:(NSString *)url placeHolderImg:(NSString *)placeHolderImgName playerOption:(YLPlayerOption * _Nullable)playerOption{
...
-(void)setUpNetShowView
{
    ...
    _netShowView = [[YLNetNotiView alloc]init];
...
-(void)setUpLoadingView
{
    ...
    _loadingView = [[YLLoadingView alloc]init];
...
-(void)setUpCoverView
{
    ...
    //在coverView上面添加返回按钮
    self.backBtn = [[YLMediaButton alloc]init];
    ...
    //在coverView上面添加全屏按钮
    self.fullScreenBtn = [[YLMediaButton alloc]init];
    ...
    //在coverView上面添加播放按钮
    _playBtn = [[YLMediaButton alloc]init];
    ...
    //在coverView上面添加锁定按钮
    _btnLock = [[YLMediaButton alloc]init];
    ...
    [_progressView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerY.equalTo(self.fullScreenBtn);//progressView向下一个像素
    }];
    ...
    [_sliderView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.width.equalTo(self.progressView);
...
-(void)setUpPlayerOption
{
    if (!self.playerOption) {
        self.playerOption = [[YLPlayerOption alloc]init];
    }
}

#pragma mark - 点击了全屏按钮
- (void)fullScreen:(YLMediaButton *)sender
{
    NSLog(@"您点击了全屏按钮");
...
#pragma mark - 播放和暂停
-(void)playOption:(YLMediaButton *)sender{
...
-(void)RecordThePropertiesOfThePlayer
{
    ...
    if (self.playerBaseView) {
        YLInterfaceOrientationType directionType = [self.playerOption getCurrentScreenDirection];
...
-(void)setUpPlayerView
{
    ...
    [self.player setPlayerOptionIntValue:20* 1024 *1024 forKey:@"max-buffer-size"];
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    if ([touch.view isKindOfClass:[YLMediaButton class]]||[touch.view isKindOfClass:[UISlider class]]){
        return NO;
    }
    return YES;
}
