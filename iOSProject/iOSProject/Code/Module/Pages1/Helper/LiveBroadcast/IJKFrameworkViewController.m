...
#import "YLNetNotiView.h"

@interface IJKFrameworkViewController ()<playerfullBtnOrBackBtnClickDelegate,playerPlayStatesDelegate,playerPlayLoadStatesDelegate,playerPlayBackStateDidChangeDelgate>

@property(nonatomic,strong)YLPlayerView *playerShowView;
@property(nonatomic,strong)NSTimer *timer;
@property(nonatomic,strong)YLPlayerOption *playerOptions;//存放视频播放器的一些属性
@property(nonatomic,strong)YLNetNotiView *netShowView;//如果进来界面，没网或者4G网，用这个显示
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    else
    {
        ...
        if (delegate.netWorkStatesCode == AFNetworkReachabilityStatusNotReachable) {
            [self setUpNetShowViewWithNetWorkNotiViewType:YLNetNotiViewTypeOfNoNetWork];
        }
        else if (delegate.netWorkStatesCode == AFNetworkReachabilityStatusReachableViaWWAN) {
            [self setUpNetShowViewWithNetWorkNotiViewType:YLNetNotiViewTypeOfBecomeWWAN];
        }
...
- (void)viewDidLoad {
    ...
    self.playerOptions = [[YLPlayerOption alloc]init];
    YLInterfaceOrientationType directionType = [self.playerOptions getCurrentScreenDirection];
...
-(void)setUpNetShowViewWithNetWorkNotiViewType:( YLNetNotiViewType )netWorkNotiViewType
{
    AppDelegate *delegate = (AppDelegate *)[UIApplication sharedApplication].delegate;
    ...
    if (!self.netShowView) {
        ...
        self.netShowView = [[YLNetNotiView alloc]init];
    ...
    if (netWorkNotiViewType == YLNetNotiViewTypeOfNoNetWork || netWorkNotiViewType == YLNetNotiViewTypeOfBecomeWWAN) {
        [self.netShowView showNetNotiViewWithType:netWorkNotiViewType];
    }
    else
    {
        if (delegate.netWorkStatesCode == AFNetworkReachabilityStatusNotReachable) {
            [self.netShowView showNetNotiViewWithType:YLNetNotiViewTypeOfNoNetWork];
        }
        if (delegate.netWorkStatesCode == AFNetworkReachabilityStatusReachableViaWWAN) {
            [self.netShowView showNetNotiViewWithType:YLNetNotiViewTypeOfBecomeWWAN];
        }
    }
    //点击了确定按钮
    self.netShowView.btnClickblock = ^{
        ...
        else
        {
            ...
            {
                YLInterfaceOrientationType directionType = [weakSelf.playerOptions getCurrentScreenDirection];
...
-(void)addNotiFicationEvent
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(playerRegistActiveNotificationEvent:) name:UIApplicationWillResignActiveNotification
                                               object:nil];
    //变成第一响应者的时候的通知方法
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(playerBecomeActiveNotificationEvent:) name:UIApplicationDidBecomeActiveNotification
                                               object:nil];
    //网络状态发生变化的时候的通知方法
    [[NSNotificationCenter defaultCenter]addObserver:self
...
-(void)createPlayerViewWithScreenDirection:(YLInterfaceOrientationType)screenDirection isPlaying:(BOOL)isPlaying currentTime:(NSTimeInterval)playerCureentTime
{
    ...
    if (screenDirection == YLInterfaceOrientationPortrait) {
        double resultWithWToH = KScreenWidth/KScreenHeight> 1? KScreenHeight/KScreenWidth : KScreenWidth/KScreenHeight;
        CGRect frame = CGRectMake(0, kNavigationHeight, KScreenWidth, KScreenWidth * resultWithWToH * 1.3);
        self.playerShowView = [[YLPlayerView alloc]initWithFrame:frame delegate:self url:self.playUrl placeHolderImg:@"placeHolderbg" playerOption:self.playerOptions];
         //NSLog(@"frame大小为11111：  %@",NSStringFromCGRect(self.playerShowView.frame));
    }
    else if (screenDirection == YLInterfaceOrientationLandscapeLeft) {
        NSNumber * value  = [NSNumber numberWithInt:UIInterfaceOrientationLandscapeLeft];
        [[UIDevice currentDevice] setValue:value forKey:@"orientation"];
        CGRect frame = self.view.bounds;
        self.playerShowView = [[YLPlayerView alloc]initWithFrame:frame delegate:self url:self.playUrl placeHolderImg:@"placeHolderbg" playerOption:self.playerOptions];
    }
    else if (screenDirection == YLInterfaceOrientationLandscapeRight) {
        NSNumber * value  = [NSNumber numberWithInt:UIInterfaceOrientationLandscapeRight];
        [[UIDevice currentDevice] setValue:value forKey:@"orientation"];
        CGRect frame = self.view.bounds;
        self.playerShowView = [[YLPlayerView alloc]initWithFrame:frame delegate:self url:self.playUrl placeHolderImg:@"placeHolderbg" playerOption:self.playerOptions];
    }
    else  if ((screenDirection == YLInterfaceOrientationPortraitUpsideDown) ||(screenDirection == YLInterfaceOrientationUnknown )) {
        double resultWithWToH = KScreenWidth/KScreenHeight> 1? KScreenHeight/KScreenWidth : KScreenWidth/KScreenHeight;
        CGRect frame = CGRectMake(0, kNavigationHeight, KScreenWidth, KScreenWidth * resultWithWToH * 1.3);
        self.playerShowView = [[YLPlayerView alloc]initWithFrame:frame delegate:self url:self.playUrl placeHolderImg:@"placeHolderbg" playerOption:self.playerOptions];
    }
   // NSLog(@"frame大小为：  %@",NSStringFromCGRect(self.playerShowView.frame));
    ...
    self.playerShowView.netShowView.btnClickblock = ^{
        //如果当前显示的是无网界面
        if (weakSelf.playerShowView.netShowView.netWorkNotiViewType == YLNetNotiViewTypeOfNoNetWork) {
            //没有网，点击了也不会有反应
            if (delegate.netWorkStatesCode == AFNetworkReachabilityStatusNotReachable) {
                return ;
            }
        ...
        //流量提醒界面,点击之后直接恢复状态
        else if (weakSelf.playerShowView.netShowView.netWorkNotiViewType == YLNetNotiViewTypeOfBecomeWWAN)
        {
...
-(void)optionsWithFullBtnClick
{
    ...
    if (directionType == YLInterfaceOrientationPortrait) {
        directionType = YLInterfaceOrientationLandscapeRight;
    }
    else if (directionType == YLInterfaceOrientationLandscapeRight) {
        directionType = YLInterfaceOrientationPortrait;
    }
    else  if (directionType == YLInterfaceOrientationLandscapeLeft) {
        directionType = YLInterfaceOrientationPortrait;
    }
    else  if ((directionType == YLInterfaceOrientationUnknown)||(directionType == YLInterfaceOrientationPortraitUpsideDown)) {
        directionType = YLInterfaceOrientationPortrait;
    }
    self.playerOptions.screenDirection = directionType;
    
    if (directionType ==  YLInterfaceOrientationLandscapeRight ) {//小屏->全屏
...
-(void)playerMPMovieLoadStateStalled
{
    ...
    if (delegate.netWorkStatesCode == AFNetworkReachabilityStatusNotReachable) {
        self.playerShowView.netShowView.netWorkNotiViewType = YLNetNotiViewTypeOfNoNetWork;
        [self.playerShowView.netShowView showNetNotiViewWithType:YLNetNotiViewTypeOfNoNetWork];
    }
}
...
-(void)playerNetworkReachabilityStatusReachableViaWWAN
{
    ...
    {
        //使用的时候，需要记录一下视频播放器的属性，这个已经在YLPlayerView里面做好了
        //先把视频暂停，虽然暂停了，但是这个时候要是有缓冲的
        [self.playerShowView pauseWithoutRecoder];
        
        
        //显示流量提醒界面
        self.playerShowView.netShowView.netWorkNotiViewType = YLNetNotiViewTypeOfBecomeWWAN;
        [self.playerShowView.netShowView showNetNotiViewWithType:YLNetNotiViewTypeOfBecomeWWAN];
    }
...
-(void)playerNetworkReachabilityStatusNotReachable
{
    ...
    if (self.playerOptions.isBeingAppearState) {
        ...
        {
            //没网的时候，需要记录一下视频播放器的属性，这个已经在YLPlayerView里面做好了
...
- (void)statusBarOrientationChange:(NSNotification *)notification
{
    ...
    else
    {
        ...
        if (orientation == UIInterfaceOrientationLandscapeRight) // home键靠右
        {
            ...
            self.playerShowView.playerOption.screenDirection = YLInterfaceOrientationLandscapeRight;
        ...
        {
            self.playerShowView.playerOption.screenDirection = YLInterfaceOrientationLandscapeLeft ;
        ...
        else if (orientation == UIInterfaceOrientationPortrait)
        {
            ...
            self.playerShowView.playerOption.screenDirection = YLInterfaceOrientationPortrait;
        ...
        {
            self.playerShowView.playerOption.screenDirection = YLInterfaceOrientationPortraitUpsideDown;
        }
        else if (orientation == UIInterfaceOrientationUnknown) {
            self.playerShowView.playerOption.screenDirection = YLInterfaceOrientationUnknown;
        }
        self.playerOptions = self.playerShowView.playerOption;
    }
}
...
-(void)playerRegistActiveNotificationEvent:(NSNotification *)sender
{
    ...
    else
    {
        ...
        //在这个代理方法执行出来之前，YLPlayerView已经记录了视频播放器的一些属性
        //把视频播放器进行暂停
        [self.playerShowView pauseWithoutRecoder];
    }
...
-(void)RestoreTheAtatusOfPlayer
{
    ...
    YLInterfaceOrientationType directionType = self.playerOptions.screenDirection;
    ...
    else if ((directionType == YLInterfaceOrientationPortraitUpsideDown) ||(directionType == YLInterfaceOrientationUnknown )) {
        NSNumber * value  = [NSNumber numberWithInt:UIDeviceOrientationPortrait];
        [[UIDevice currentDevice] setValue:value forKey:@"orientation"];
        double resultWithWToH = KScreenWidth/KScreenHeight> 1? KScreenHeight/KScreenWidth : KScreenWidth/KScreenHeight;
        CGRect frame = CGRectMake(0,kNavigationHeight, KScreenWidth, KScreenWidth * resultWithWToH * 1.3);
